...
import cz.plesioEngine.textures.TextureData;
...
public class Loader {
    ...
    public void addInstancedAtrribute(int vao, int vbo, int attribute, int dataSize,
    ...
    public RawModel loadToVAO(float[] positions, float[] textureCoords, int[] indices, float tangents[], float[] normals) {
        int vaoID = createVAO();
        bindIndicesBuffer(indices);
        storeDataInAttributeList(0, 3, positions);
        storeDataInAttributeList(1, 2, textureCoords);
    ...
    public int loadTexture(String fileName) {
        ...
        try {
            ...
            if(GLContext.getCapabilities().GL_EXT_texture_filter_anisotropic){
                ...
                ConsoleOutput.appendToLog("Anisitropic filtering unsupported!", ConsoleOutput.LogType.ERR);
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public int LoadFontTextureAtlas(String fileName) {
        ...
        try {
            texture = TextureLoader.getTexture("PNG",
                    new FileInputStream("res/font/" + fileName + ".png"));
            GL30.glGenerateMipmap(GL11.GL_TEXTURE_2D);
            GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR_MIPMAP_LINEAR);
